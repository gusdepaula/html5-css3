...
export default class App extends React.Component {
  ...
  render() {
    ...
    return (
      <section dir={isRtl ? "rtl" : "ltr"} className="mdc-typography">
        {shouldReinit ? null : (
          <TopAppBar
        ...
        <div
          className={classnames("mdc-top-app-bar--fixed-adjust", {
  ...
  renderControls() {
    ...
    return (
      <div className="demo-controls-box">
        <h3 className="mdc-typography--title">Controls</h3>

        {checkboxes.map((checkbox, key) =>
          this.renderControlCheckbox(checkbox, key)
        )}
      </div>
    );
  }
  ...
  renderControlCheckbox({ title, attr, disabled = false }, key) {
    ...
    return (
      <div
        key={key}
        className={`demo-control-checkbox ${
          disabled ? "demo-control-checkbox--disabled" : ""
        }`}
      >
        <label htmlFor={labelId} className="demo-control-checkbox__label">
          {title}
        </label>
        <input
          type="checkbox"
